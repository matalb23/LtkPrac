...
export class DbService {
  ...
  existServicio(codigo: number) {
    //var respuesta = new Subject<boolean>();
   //var respuesta = new BehaviorSubject(Boolean);
    var b
  ...
  // Get single object
  /*getServicio(Codigo): Promise<Servicio> {
    return this.storage.executeSql('SELECT * FROM servicios WHERE id = ?', [Codigo]).then(res => { 
      return {
        Codigo:res.rows.item(0).codigo,
        Cliente:res.rows.item(0).cliente,
        ClienteRazonSocial:res.rows.item(0).clienteRazonSocial,
      }
    });
  }*/
  ...
  update(s: Servicio) {
    let data = [s.fechaInicio, s.fechaFin, s.calado_Popa, s.calado_Proa];
    console.log("updatesong_data");
    console.log(data);
    console.log(s.codigo);
    return this.storage.executeSql(`UPDATE servicios SET fechaInicio = ?, fechaFin = ?,calado_Popa = ?,calado_Proa = ?  WHERE codigo = ${s.codigo}`, data)
      .then(data => {
        this.getServicios();
      })
  }
